...
int main(int argc, char** argv) {
  ...
  auto* dut = new Vcpu_top;
  auto* trace = new VerilatedVcdC;
  dut->trace(trace, 5);
  trace->open("waveforms/cpu_wave.vcd");
  ...
  if (dut->done && dut->debug_reg_data == 55 && dut->debug_mem_data == 55) {
    VL_PRINTF("[PASS] Verilator single-cycle run complete. x11=%u mem[0x100]=%u cycles=%llu\n",
              dut->debug_reg_data, dut->debug_mem_data,
              static_cast<unsigned long long>(dut->cycle_count));
  } else {
    VL_PRINTF("[FAIL] Verilator single-cycle run mismatch. done=%u x11=%u mem[0x100]=%u cycles=%llu\n",
